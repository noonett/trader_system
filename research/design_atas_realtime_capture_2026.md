...
>
> **当前版本**：v0.2（2026-05-08 — 组件 1+2 已实现并通过 code review；组件 3 待定）
>
...
| # | 组件 | 语言/平台 | 复杂度 | 当前状态 |
|---|------|-----------|--------|----------|
| 1 | **ATAS C# 自定义指标（SigmaBridge）** | C# / .NET / ATAS SDK | 中高 | ✅ 已实现（`atas-plugin/SigmaBridge/`）— 待用户 Phase 1 实盘验证 |
| 2 | **本地中继服务（sigma-relay）** | Python | 中 | ✅ 已实现（`tools/sigma-relay/`）— 22 个测试通过 |
| 3 | **Agent 侧 trade-draft 生成器** | TypeScript（webui 现有栈） | 中低 | 未开始 — v0.2 采用简单路径：sigma-relay 直接 git commit draft |
...
**文件位置**：`atas-plugin/SigmaBridge/SigmaBridge.cs`（✅ 已实现，~180 行）
...
**文件位置**：`tools/sigma-relay/`（✅ 已实现，含 server/config/persistence/screenshot/notify/file_watcher + 22 个测试）
...
| # | 名称 | 类型 | 位置 | 状态 |
|---|------|------|------|------|
| 1 | **SigmaBridge** | ATAS C# DLL | `atas-plugin/SigmaBridge/` | ✅ ~180 行（含 static dedup + FireAndForget） |
| 2 | **sigma-relay** | Python 常驻服务 | `tools/sigma-relay/` | ✅ ~600 行主逻辑 + ~300 行测试 |
| 3 | **draft_generator** | Python | `tools/sigma-relay/draft_generator.py` | ✅ ~260 行（YAML frontmatter + 模板） |
| 4 | **通知推送模块** | Python | `tools/sigma-relay/notify.py` | ✅ ~180 行（toast/企业微信/PushPlus/Server酱） |
| 5 | **截图模块** | Python | `tools/sigma-relay/screenshot.py` | ✅ ~106 行（mss 截图 + 归档） |
| 6 | **事件持久化** | Python | `tools/sigma-relay/persistence.py` | ✅ ~130 行（JSONL 日志 + 重启恢复） |
| 7 | **position_core** | Python | `tools/sigma-relay/position_core.py` | ✅ ~100 行（共享仓位逻辑） |
| 8 | **WebUI draft indicator**（可选） | TypeScript | `webui/src/` | 未开始 |
...
### Phase 1：验证 ATAS C# HTTP 发送可行性（⏳ 待用户实盘验证）

1. ✅ 创建 SigmaBridge C# 指标（`atas-plugin/SigmaBridge/`）
2. ⏳ 编译 DLL 并挂载到 ATAS 图表 → 确认事件在实盘/模拟盘中触发
3. ⏳ 验证 `HttpClient.PostAsync` 到 localhost 是否被沙箱阻止

**如果 HTTP 被阻止**：配置 `file_watcher_dir` 即可切换为文件监控模式（代码已实现，无需改动）。

### Phase 2：sigma-relay 本地服务（✅ 已完成）

1. ✅ FastAPI 服务 + 5 个端点（trade-event / position-closed / order-update / screenshot / health）
2. ✅ Partial fill 聚合逻辑 + opening/reducing fill 区分
3. ✅ Draft markdown 模板生成（frontmatter 对齐 TEMPLATE.md 格式）
4. ✅ `git add + commit` 自动化（pathspec 提交，不连带无关文件）
5. ✅ 事件持久化 + 重启恢复（JSONL 日志，3 天 lookback）
6. ✅ 截图自动捕获 + 归档（mss 纯 Python）
7. ✅ 通知推送（Windows Toast + 企业微信 + PushPlus + Server酱）
8. ✅ 文件监控退化方案（watchdog）
9. ✅ 线程安全（threading.Lock 保护 open_positions）
10. ✅ 22 个测试（单元 + 集成）

### Phase 3：端到端测试（⏳ 待用户 Windows 环境验证）

1. ⏳ ATAS 模拟盘下单 → SigmaBridge 捕获 → relay 接收 → draft 生成
2. ⏳ 验证 partial fills 聚合（模拟 2 手分批场景）
3. ⏳ 验证 draft 文件格式与现有 trades/ 记录兼容

### Phase 4：截图辅助 + 通知（✅ 代码已实现，⏳ 待端到端验证）

1. ✅ sigma-relay 侧 mss 截图（成交时自动 + 平仓时自动）
2. ✅ SigmaBridge 侧 nircmd 截图（可选，用户可关闭）
3. ✅ 通知推送通道实现
4. ⏳ WebUI 中 draft 标记展示（未开始）
...
*文档版本：v0.2 | 更新日期：2026-05-08 | 诚实标记：ATAS API 信息为 M 级（官方文档）；架构方案为 U 级（逻辑推演）；组件 1+2 代码已实现但未经 ATAS 实盘验证（最高风险未知数：沙箱是否允许 System.Net.Http）*

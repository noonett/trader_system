...
## v2 工程现实 caveat（First-pass review R03-Engineer 抓出来的低估）

### caveat 1：markdown + git GUI 封装是被低估 1 个数量级的工程承诺

v1 笔记写"git GUI 封装是必须的"似乎是个轻量结论。R03-Engineer 指出实战工程量被严重低估：

- **Obsidian + Obsidian Git plugin** 是社区**单人维护**（denolehov），5 年维护风险不可忽略
- **git 强审计能力**需要 6 步配置：(1) 生成 GPG key + (2) 上传 GitHub/GitLab + (3) `git config --global commit.gpgsign true` + (4) 配置私有远程 + (5) 设置 protected branch（GitHub Pro/Team plan）+ (6) 配置 `git push --force-with-lease`（防止意外覆盖）—— **对小白用户基本不会全做对**
- 调研里"git 审计 ✓" 表格是**理论上限**，**不是默认形态**——默认 Obsidian Git plugin 只做 auto-commit，不做 GPG 签名 + force-with-lease 防护
- 任何 Phase 2 Design 选 markdown + git 路径的，**必须明确标注实施门槛**：完全用 GUI（无 CLI 学习）= 中等审计；想要强审计 = 需要 6 步配置 = 已超出"小白可用"范围

**对 σ Phase 2 Design 的含义**：
- **N=1 自用语境**：Obsidian + Obsidian Git plugin 的"中等审计"已经够用，不强求 GPG 签名 + protected branch
- **未来 productization 语境**：必须把强审计做成"用户一键启用"模式，否则就是把"git 审计能力"夸大成事实

### caveat 2：AI 周期性透明读 与 "本地优先 / 中等被审视感" 的硬件冲突

§3.1 设计需求清单中"AI 周期性透明读 ≈ 中等被审视感"这一条，在工程上的**默认实现**是调云 LLM API（OpenAI / Anthropic / DeepSeek / Moonshot 等）。这与"本地优先 / 中等被审视感"原则**直接矛盾**——把 markdown 内容（含交易心理记录、个人决策模式、可能的临床敏感内容）发给云 API = 完全公开级的隐私暴露。

**真正落地"本地 + AI 周期性读" 的硬件门槛**：
- 必须 **Ollama / llama.cpp + 本地 7B-13B 模型**（如 Qwen 2.5 7B / Llama 3.1 8B）
- **32GB RAM 起步**（13B 量化模型在 16GB 上会 swap 严重）
- **20-50GB SSD 空间**用于模型权重
- **GPU 推荐**（CPU-only 推理速度可能让"周期性读"延迟到几分钟级）
- **prompt engineering 工程量 20-40 工时**（"让 LLM 读完什么都不主动做" + "非谄媚输出" + "不主动归因"约束）

**对 σ Phase 2 Design 的含义**：
- **N=1 自用语境（v0 阶段）**：默认 **完全无 AI**（与 R03-Engineer 推荐 v0 方案一致）；如用户主动想加 AI，明确两选项：(a) 接受云 API + 隐私边界透明声明；(b) 上 Ollama 本地路径 + 接受硬件要求
- **未来 productization 语境**：必须把"本地 AI vs 云 AI"作为用户**显式选择**，不能默认绑定

### caveat 3：git 强审计的"force push 仍能绕过"边界

§1.1 已经识别 git 在"本地未 push 时可以 rebase / amend / force-with-lease 静默重写"。R03-Engineer 强化这一点：**即使 push 到远程，用户仍可以**：
- 删除整个 .git 目录重新初始化（终极绕过）
- 在另一台机器上 clone + 重写 + force push（如未配 protected branch）
- 删除 GitHub repo + 重建（彻底洗白）

**含义**：**git 不是审计型介质**，是"**中等被审视感**"介质——它让事后改写**昂贵**（需要主动行为 + 知识），但不让它**不可能**。这是设计意图（让用户**有意识地**记住自己改了），不是漏洞。**Phase 2 Design 不应承诺"git = 不可篡改"** —— 这是过度承诺。

### caveat 4：Tauri / Electron / 原生桌面应用的工程量数量级差异（v0 选型相关）

R03-Engineer 提到 σ 选型时容易低估：
- **Obsidian 插件**：< 5 工时上手，但受插件 API 限制
- **Tauri 应用**：50-150 工时（Rust + 前端）
- **Electron 应用**：30-80 工时（JS + 资源占用大）
- **原生桌面（macOS Swift / Windows .NET）**：100-300 工时（跨平台 = 双倍）
- **Web App**：30-60 工时（部署 + 域名 + 隐私）
- **CLI**：20-40 工时（受众极窄）

**含义**：v0 阶段**强烈推荐 Obsidian 插件路径**（与 §三.1 设计需求清单契合 + 工程量最低）；其他形态推 Phase 3+ 或 productization 阶段。

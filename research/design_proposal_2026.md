...
>
> **与 Phase 3 的分工**：本文件是 **Design**（规范与取舍），不替代 Action 层的逐步操作说明。**Phase 3a（v0 启动硬集）已在仓库落地**：`sigma/`、`trades/TEMPLATE.md`、`reviews/`、`scripts/`、`Makefile`、红区 `pre-commit` 安装脚本等——见 `memory/stage.md` 与 §八.2。**Phase 3b/c/d**（Mobile 模板、WebUI 渲染、完整 KPI 工具等）仍按本文件阶段表推进；**本文件不展开**各 PR 的具体工程实现细节。
...
**这是一个全新项目**：老的 [decision-chain.md](../archive/deprecated_stage0/decision-chain.md) / [trade-journal-template.md](../archive/deprecated_stage0/trade-journal-template.md) / [position_sizer.py](../archive/deprecated_stage0/tools/position_sizer.py) / [trades/2026/05/](../trades/2026/05/) 都不再被沿用——v0 用调研证据重新设计这一整套，老文件按 §七处置（保留 1 份在 archive/deprecated_stage0/ 作为历史，不再纳入工作流）。
...
```
/workspace/
├── AGENTS.md                           ← 项目上下文（Cursor / Agent；原 CLAUDE.md）
├── archive/system_spec_v2.md            ← 早期规范 V2（仅历史；权威见 foundation + entry_form + 本文件）
├── README.md                            ← v0 σ 操作手册（Phase 3a PR-2 新建，可选）
...
> **v0 改动**：旧 [decision-chain.md](../archive/deprecated_stage0/decision-chain.md) Q1–Q5 是开放式问题（What/Why 主导）；v0 改为 if-then 形式（implementation intention，d=.27-.66 跨域元分析，foundation §三.6 调研 6）+ identity 第一人称（BCT 13.x）。
...
**基线自检不是 v0 启动条件**——v0 启动条件 = 写第一份盘前规则。基线自检建议在 v0 启动后首月内找一个非交易时段完成（~10 分钟，35 道题），不阻塞启动。结果存到 `memory/clinical_baseline.md`。N=1 用户已自评不属极端高风险——入门时强制做临床量表的 ROI 极低；真正需要它的时间点是"连续亏损 + 情绪波动"之后。
...
| 旧文件                                                                                             | 处置                                      | 理由                                                                                                                   |
| ----------------------------------------------------------------------------------------------- | --------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| [decision-chain.md](../archive/deprecated_stage0/decision-chain.md)（已归档）                         | **archive/deprecated_stage0/**            | 旧版是开放式 Q1-Q5（What/Why 主导），与 v3 §四.4 BCT identity + foundation §三.4 if-then 范式不一致。v0 在 sigma/decision-chain.md 重写。    |
| [trade-journal-template.md](../archive/deprecated_stage0/trade-journal-template.md)（已归档）         | **archive/deprecated_stage0/**            | 旧版是单文件模板；v0 在 trades/YYYY/MM/.md 一文聚合（盘前 if-then + 决策链 + 盘后 EMA + 异步复盘）+ sigma/post-trade-ema.md 提供 EMA 模板。          |
| [position_sizer.py](../archive/deprecated_stage0/tools/position_sizer.py)（已归档）                    | **archive/deprecated_stage0/tools/**      | 工具本身实现没问题（半 Kelly），但作为独立 Python 脚本不在 v0 工作流中——v0 把仓位计算嵌入决策链 If 5（期望值 + Q5a 来源校验）。若需可继续从归档复制使用。 |
...
| [system_spec_v2.md](../archive/system_spec_v2.md)（已归档）                                          | **archive/system_spec_v2.md**           | 早期规范 V2；已被 foundation v5 + entry_form_research v3 + 本文件（Design）取代；2026-05-07 从根目录移入 archive，文首有归档说明。               |
...
- 周批量对账的具体 CSV 格式
- ~~system_spec_v2.md 顶部的"已被取代"说明~~（已完成：文件已移入 `archive/system_spec_v2.md` 并加归档头）
...
| 2026-05-08 | v0.1（文档） | **根说明改名**：`CLAUDE.md` → `AGENTS.md`（Cursor 惯例）；更新 `.cursor/rules/sigma-meta-rule-precedence.mdc`、`design_proposal` §3.1、`archive/system_spec_v2` 归档头。 | 用户要求与 Cursor 项目结构对齐 |
| 2026-05-07 | v0.1（文档） | **`system_spec_v2.md` 归档**：从根目录移至 `archive/system_spec_v2.md`，加归档头；`CLAUDE.md` / §3.1 目录树 / §7.1 处置表 / glossary 引用同步；`archive/README.md` 增专节。 | 用户要求去除根目录老规范、避免与当前权威双轨并存 |
| 2026-05-07 | v0.1（文档） | **Action 对齐**：文首「不输出 Phase 3 Action」改为「Design 与 3a 已落地工程分工说明」；§七 表格内旧根目录链接改为 `archive/deprecated_stage0/` 实址（避免 404）。 | Phase 3a 已合并 `master` 后读者反馈 + 仓库内链接审计 |

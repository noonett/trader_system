...
- [research/design_proposal_2026.md](../research/design_proposal_2026.md) — **Phase 2 Design v0（2026-05-06）**，659 行。在 N=1 用户语境下对 12 项 D1–D12 给出推荐选择 + 主证据 + 反向证据 + 退化路径；定义 v0 sigma/ 目录结构 + 4 层工作流 + 4 周可证伪 KPI + 4→3→2→1→0 退化 + 退出协议；§七 处置旧 Stage 0 工具（用户明确不兼容）。**当前 Design 主交付**。

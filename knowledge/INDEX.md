...
- [research/entry_form_research_2026.md](../research/entry_form_research_2026.md) — Phase 2 入口形态调研整合（v2），458 行。Plan→Design 桥梁；6+1 收敛信号 + 12 项 Phase 2 Design 必决事项 + §六 N=1 vs productization 临床安全分级。
...
> ⚠️ **暂未建库（Level C 未启动）。** Phase 1 v5 列出 12 个 S 级证据组件 + 13 节 Plan 约束；Phase 2 v2 整合出 6+1 个跨调研收敛信号——这些都是 claims/ 的天然候选。建议在 Phase 2 Design 启动前后建库（每个 claim 直接对应一个 Design 决策的证据基础时使用频率最高）。

候选条目（仅列纲要，未真正建库；详见 research/foundation_2026.md 与 entry_form_research_2026.md 原文）：
...
| `behav-004` | 痛苦驱动型交易（DSM-5 #5）= σ 系统最危险的过渡机制 | entry_form_research §6.3 | 候选 |
...
| `system-002` | 早晨规则书写 ROI 数量级最高（0-5 工时 vs 盘中浮窗 80-160 工时）| entry_form_research §四.5 | 候选 |
| `system-003` | 完全不用 AI 前台是合法选项（对临床高风险用户必需）| entry_form_research §6.1 + notes/12 | 候选 |
...
| `tech-001` | A 股 9:20-9:25 集合竞价不可撤单 = 市场制度免费提供的 commitment device | entry_form_research §四.7 | 候选 |
| `me-001` | 用户自评不在极端高风险 → 临床安全自检不作 lockout gate | entry_form_research §6.1 + 用户输入 2026-05-05 | 候选 |

## 待验证队列 (testing/queue.md)

暂无（Phase 2 Design 启动后可建立——每个 Plan 级约束都需要 N=1 + 4 周可证伪 KPI；详见 entry_form_research §四.3 可证伪性约束）。
...
| **2026-05-05** | **Level A sync 更新**：加入 research/ 章节链接（顶层综合文档 + 12 篇 notes + Review 档案）；claims/ 候选条目纲要列出 12 项（来自 Phase 1 v5 + Phase 2 v2 已合内容）；学习源类别按调研笔记归类。**未建实际 claims 库（Level C 推迟到 Phase 2 Design 前后启动）**。 |
